...
;  If GSTATE is nil, return the default-gcontext for VIEW.  Otherwise, compute
;  the working gcontext by combining the default gcontext with gstate.
...
(DEFMACRO prepare-gcontext (view gstate  gcontext)
  ...
  `(LET ((ga (slot-value ,gstate 'gstate-array)))
      (WHEN (ELT ga foreground) (SETF (gcontext-foreground ,gcontext) (CADR (ELT ga foreground))))
      (WHEN (ELT ga background)  (SETF (gcontext-background ,gcontext) (CADR (ELT ga background))))
      (WHEN (ELT ga line-style ) (SETF (gcontext-line-style ,gcontext) (ELT ga line-style)))
      (WHEN (ELT ga line-width) (SETF (gcontext-line-width ,gcontext) (floor (* (view-scale ,view)(ELT ga line-width)))))
      (when (elt ga function )  (SETF (gcontext-function ,gcontext) (elt ga  function)))
      (when (elt ga ts-x )  (SETF (gcontext-ts-x ,gcontext)  (elt ga  ts-x)))
      (when (elt ga ts-y )   (SETF (gcontext-ts-y ,gcontext) (elt ga ts-y)))
      (when (elt ga arc-mode)  (SETF (gcontext-arc-mode ,gcontext) (elt ga  arc-mode)))
      (when (elt ga cap-style )  (SETF (gcontext-cap-style ,gcontext) (elt ga  cap-style)))
      (IF (elt ga clip-mask )  (SETF (gcontext-clip-mask ,gcontext) (elt ga  clip-mask))
      ...
      (when (elt ga tile )  (SETF (gcontext-tile ,gcontext)
	    (LET ((image (CADR (elt ga  tile))))
		  ...
		  (contact-image-mask ,view image :foreground (OR (gstate-foreground gstate) 1)
				      :background (OR (gstate-background gstate) 0))
		  (contact-image-pixmap ,view image)))))
      (when (elt ga dash-offset )  (SETF (gcontext-dash-offset ,gcontext) (elt ga dash-offset)))
      (when (elt ga dashes )  (SETF (gcontext-dashes ,gcontext) (elt ga  dashes)))
      (when (elt ga fill-rule )  (SETF (gcontext-fill-rule ,gcontext) (elt ga  fill-rule)))
      (when (elt ga fill-style )  (SETF (gcontext-fill-style ,gcontext) (elt ga  fill-style)))
      (when (elt ga font )  (SETF (gcontext-font ,gcontext) (elt ga  font)))
      (when (elt ga stipple-pix ) (setf (gcontext-stipple ,gcontext) (elt ga stipple-pix )))
      (when (elt ga join-style )  (SETF (gcontext-join-style ,gcontext) (elt ga  join-style)))
      (when (elt ga stipple )  (SETF (gcontext-stipple ,gcontext)
  ...
  (with-gstate-value-place
	...
	(IF (elt ga clip-mask )
	    ...
	    (WHEN (OR (elt ga stipple )(elt ga tile )(elt ga stipple-pix ))
	      ...
	      (when (elt ga ts-y )  (SETF (gcontext-ts-y new-gcontext)  (elt ga  ts-y)))))
...
(defmethod view-draw-arc ((view view) x-min y-min width height
  ...
  (when (and (valid-xcoord x-min) (valid-xcoord y-min) (valid-xdistance width) (valid-xdistance height))
    (draw-arc view
              (if gstate (working-gcontext view gstate) (slot-value view 'default-gcontext))
              x-min (- y-min height) width height angle1 angle2)))


(defmethod view-draw-filled-arc ((view view) x-min y-min width height
			       angle1 angle2 &optional gstate)
  (declare (type wcoord x-min y-min width height))  (declare (type angle angle1 angle2))

  (multiple-value-setq (x-min y-min) (transform-point view x-min y-min))
  (multiple-value-setq (width height) (view-scale-point view width height))
  (when (and (valid-xcoord x-min) (valid-xcoord y-min) (valid-xdistance width) (valid-xdistance height))
    (draw-arc view (if gstate (working-gcontext view gstate) (slot-value view 'default-gcontext))
...
(defmethod view-draw-line ((view view) start-x start-y end-x end-y
  ...
  (when (and (valid-xcoord start-x)
             ...
             (valid-xcoord end-x)
             (valid-xcoord end-y))
    (draw-line view (if gstate (working-gcontext view gstate) (slot-value view 'default-gcontext))
...
(defmethod view-draw-rectangle ((view view) x-min y-min width height
  ...
  (WHEN (AND (valid-xcoord x-min)		; Make sure rect is within the universe
    ...
    (draw-rectangle view (if gstate (working-gcontext view gstate) (slot-value view 'default-gcontext))	; Draw it
...
(defmethod view-draw-text ((view view) x-origin y-origin text
 ...
 (MULTIPLE-VALUE-BIND (x y)
    ...
    (WHEN (AND (valid-xcoord x) (valid-xcoord y))
      (draw-glyphs view (if gstate (working-gcontext view gstate) (slot-value view 'default-gcontext))
			 x y text))))
...
(defmethod view-draw-image-text ((view view) x-origin y-origin text
 ...
 (MULTIPLE-VALUE-BIND (x y)
      (transform-point view x-origin y-origin)
    (WHEN (AND (valid-xcoord x) (valid-xcoord y))
      (draw-image-glyphs view (if gstate (working-gcontext view gstate) (slot-value view 'default-gcontext))
			 x y text))))
...
(defmethod view-draw-image-char ((view view) x-origin y-origin text
 ...
 (MULTIPLE-VALUE-BIND (x y)
      (transform-point view x-origin y-origin)
    (WHEN (AND (valid-xcoord x) (valid-xcoord y))
      (draw-image-glyph view (if gstate (working-gcontext view gstate) (slot-value view 'default-gcontext))
			 x y text))))
